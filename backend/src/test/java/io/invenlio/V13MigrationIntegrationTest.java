...
@Testcontainers
class V13MigrationIntegrationTest {
    @Container static final PostgreSQLContainer DB=new PostgreSQLContainer("postgres:17.6-alpine");

    @Test void upgradeFromV12PreservesPackedDataAndAddsShippingSchema() throws Exception {
        ...
        Flyway.configure().dataSource(DB.getJdbcUrl(),DB.getUsername(),DB.getPassword())
            .schemas(schema).defaultSchema(schema).target("12").load().migrate();
        ...
        try(var connection=DriverManager.getConnection(DB.getJdbcUrl(),DB.getUsername(),DB.getPassword());
            ...
            statement.execute("INSERT INTO organizations(id,name,slug,status,default_locale,default_timezone,default_currency,created_at,updated_at,version) VALUES('"
                +tenant+"','Upgrade','shipping-upgrade','ACTIVE','en','UTC','EUR',now(),now(),0)");
        }
        ...
        try(var connection=DriverManager.getConnection(DB.getJdbcUrl(),DB.getUsername(),DB.getPassword());
            ...
            try(var rows=statement.executeQuery("SELECT count(*) FROM shipments")) {
                rows.next(); assertThat(rows.getInt(1)).isZero();
            }
            try(var rows=statement.executeQuery("SELECT count(*) FROM shipment_packages")) {
                rows.next(); assertThat(rows.getInt(1)).isZero();
            }
        }
    }
